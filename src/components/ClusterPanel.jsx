 ...
 * Desktop: slides in from the right side of the map (absolute positioned).
...
export default function ClusterPanel({ events, onClose, onEventClick }) {
  ...
  return (
    <>
      ...
      {/* Panel — desktop: right rail; mobile: bottom sheet */}
      <div className={`
        cluster-panel
        absolute z-[400]
      ...
      `}>
        {/* Header */}
        <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b border-gray-100 dark:border-gray-700 flex-shrink-0">
          <div>
            <h3 className="text-sm font-bold text-gray-900 dark:text-white">
              {events.length} Events here
            </h3>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Tap an event for details</p>
          </div>
          <button
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 transition"
          >
          ...
          {events.map((event) => {
            ...
            return (
              <button
                ...
                className="w-full flex items-center gap-3 p-2.5 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 active:bg-gray-100 dark:active:bg-gray-600 transition text-left"
              >
                {/* Thumbnail */}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {venue?.name && (
                    <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5 truncate">{venue.name}</p>
                  )}
                  {date && (
                    <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{date}</p>
                  )}
                  <p className="text-xs font-semibold text-indigo-500 mt-1">{price}</p>
                </div>
