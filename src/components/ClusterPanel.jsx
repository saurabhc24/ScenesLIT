 ...
 * Dark mode applies on both mobile and desktop.
 */
export default function ClusterPanel({ events, onClose, onEventClick }) {
  ...
  return (
    <>
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b border-gray-100 dark:border-gray-600 flex-shrink-0">
          <div>
            <h3 id="cluster-panel-title" className="text-sm font-bold text-gray-900 dark:text-white">
              {events.length} Events here
            </h3>
            <p className="text-xs text-gray-600 dark:text-gray-400 mt-0.5">Tap an event for details</p>
          </div>
          <button
            onClick={onClose}
            aria-label="Close"
            className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50"
          >
          ...
          {events.map((event) => {
            ...
            return (
              <button
                ...
                className="w-full flex items-center gap-3 p-2.5 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 active:bg-gray-100 dark:active:bg-gray-600 transition text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50"
              >
                {/* Thumbnail */}
                <div className="w-14 h-14 rounded-xl overflow-hidden flex-shrink-0 bg-gray-100 dark:bg-gray-700" aria-hidden="true">
                  {event.image_url ? (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {venue?.name && (
                    <p className="text-xs text-gray-600 dark:text-gray-300 mt-0.5 truncate">{venue.name}</p>
                  )}
                  {date && (
                ...
                {/* Arrow */}
                <svg className="w-4 h-4 text-gray-300 dark:text-gray-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
